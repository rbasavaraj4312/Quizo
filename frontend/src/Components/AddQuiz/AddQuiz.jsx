...
const AddQuiz = ({ userDetails, server }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${server}/create-quiz`, quizData);

      if (response.status !== 200 && response.status !== 201) {
        throw new Error(
          `Failed to create quiz. Server responded with status: ${response.status}`
        );
      }
